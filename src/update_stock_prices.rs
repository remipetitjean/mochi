...
where T: for<'a> Deserialize<'a> + std::fmt::Debug
{
    ...
    if !code.is_success() {
        return Err(ApiError::Reqwest {code: code.into(), text: text.to_string()} );
    }
    ...
    match err {
        ...
        Err(err) => { return Err(ApiError::Unhandled { text: err.to_string() }) }
    };
}
...
async fn main() {
    ...
    // retrieve companies
    let stocks = Stock::select(pool.to_owned())
        .await
        .unwrap();
...
fn add_days(date: NaiveDate, num_days: u64) -> NaiveDate {
    date.clone().checked_add_days(Days::new(num_days)).expect("should have a value")
}
...
async fn get_inception_date(symbol: &str) -> Result<NaiveDate, ApiError> {
    ...
    let inception_date_url = format!("{}?symbol={}&interval=1day", inception_date_endpoint, symbol);
    let inception_date_model = json_from_api::<InceptionDateModel>(&inception_date_url).await.unwrap();
    Ok(inception_date_model.datetime)
}

async fn retrieve_and_store_stock_price(pool: PgPool, symbol: &str, start_date: NaiveDate, end_date: NaiveDate) -> Result<(), LoaderError> {
    println!("Retrieving prices for {} [{} - {}]", symbol, start_date, end_date);
    let stock_prices_endpoint = "https://api.twelvedata.com/time_series";
    let stock_prices_url = format!("{}?symbol={}&interval=1day&start_date={}&end_date={}", stock_prices_endpoint, symbol, start_date, end_date);
    let stock_prices = json_from_api::<StockPriceModel>(&stock_prices_url).await.unwrap();
    ...
    for stock_price in stock_price_values {
        let db_stock_price = StockPrice {
            ...
            volume: stock_price.volume.parse::<i32>().unwrap(),
        };
    ...
    println!("{:?}", db_stock_prices);
    StockPrice::insert_many(pool, db_stock_prices).await.unwrap();
...
async fn retrieve_and_store_stock_price_since_inception(pool: PgPool, symbol: &str) -> Result<(), ApiError> {
    ...
    while end_date <= today {
        retrieve_and_store_stock_price(pool.to_owned(), symbol, start_date, end_date).await.unwrap();
        start_date = end_date;
